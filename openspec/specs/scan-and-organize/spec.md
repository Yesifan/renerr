...
- **AND** a later scan reads the configured library path root
...
### Requirement: 整理结束后自动刷新配置的 Library Path

系统 SHALL automatically enqueue one deduplicated `scan_library_path` task for the configured library path after an `execute_rename_plan` task finishes, regardless of whether all rows succeeded or some rows failed. `organizeTargetPath` is an output-only path for this library and is not a second scan scope.

#### Scenario: 自动扫描 Library Path

- **WHEN** an `execute_rename_plan` task finishes
- **THEN** system MUST collect source and target directories from selected plan items for the execution summary
- **AND** system MUST enqueue one `scan_library_path` task for the configured library path
- **AND** the scan MUST use the configured library root as the source of truth

#### Scenario: 扫描任务去重

- **WHEN** the configured library path already has a queued or running scan task
