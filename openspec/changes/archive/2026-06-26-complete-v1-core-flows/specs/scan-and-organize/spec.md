...
- **WHEN** 扫描仍存在于远端的 `failed` item
- **THEN** 系统 MUST 跳过该 item 的识别、内部检查和自动整理
- **AND** 用户手动扫描该 item 时 MUST 允许刷新内部视频数量和模板符合度统计
...
#### Scenario: 手动扫描失败 item

- **WHEN** 用户手动扫描 `failed` item
- **THEN** 系统 MUST 刷新其内部视频数量和模板符合度统计
- **AND** 系统 MUST 保持其 `failed` 状态，直到用户重新指定 identity 或重新提交整理计划
