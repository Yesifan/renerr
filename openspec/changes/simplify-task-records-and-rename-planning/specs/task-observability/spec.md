...
**Reason**: 用户可见任务事件改由 `task_detail_lines` 提供，服务器诊断日志改由 pino 文件日志提供，不再维护数据库 logs 作为任务详情数据源。

**Migration**: Replace DB task logs with task detail lines. Emit diagnostics through pino.

### Requirement: Execution records 保留 row 级 MOVE 结果

**Reason**: 前端任务详情只需要英文运行记录字符串列表，不再需要结构化 row-level execution records。

**Migration**: Persist row state in `rename_plan_items.status`, summarize counts in `tasks.resultSummaryJson`, and append user-visible row result messages to `task_detail_lines`.

### Requirement: 全局日志流

**Reason**: 数据库 logs 表和全局日志页不再作为产品能力保留。应用日志由 pino 写入 stdout，并由 Docker logging driver 或宿主进程管理日志保留和轮转。

**Migration**: Remove global log API/UI backed by the database logs table. Use pino stdout/container logs for diagnostics.

### Requirement: 日志和记录存储控制

**Reason**: 数据库 logs 和 execution records 被移除后，不再需要针对这两类表的存储控制规则。Pino 不写应用管理的日志文件。

**Migration**: Apply retention only to task entities and task detail lines if needed. Pino stdout retention/rotation is handled by Docker logging driver or the host process manager.
